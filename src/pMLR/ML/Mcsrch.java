...
public class Mcsrch
{
	...
	private double lbfgs_stpmax, lbfgs_stpmin, lbfgs_gtol;
	...
	public void mcsrch ( int n , double[] x , double f , double[] g , double[] s , int is0 , double[] stp , double ftol , double xtol , int maxfev , int[] info , int[] nfev , double[] wa , double _lbfgs_gtol , double _lbfgs_stpmin , double _lbfgs_stpmax )
	{
		lbfgs_gtol = _lbfgs_gtol;
		lbfgs_stpmin = _lbfgs_stpmin;
		lbfgs_stpmax = _lbfgs_stpmax;
		...
		{
			infoc[0] = 1;
			if ( n <= 0 || stp[0] <= 0 || ftol < 0 || lbfgs_gtol < 0 || xtol < 0 || lbfgs_stpmin < 0 || lbfgs_stpmax < lbfgs_stpmin || maxfev <= 0 ) 
				return;
			...
			if ( dginit >= 0 )
			{
				System.err.println( "The search direction is not a descent direction." );
				return;
			}
		...
		while ( true )
		{
			if ( info[0] != -1 )
			{
				...
				stp[0] = Math.max ( stp[0] , lbfgs_stpmin );
				stp[0] = Math.min ( stp[0] , lbfgs_stpmax );
			...
			if ( stp[0] == lbfgs_stpmax && f <= ftest1 && dg <= dgtest ) info[0] = 5;

			if ( stp[0] == lbfgs_stpmin && ( f > ftest1 || dg >= dgtest ) ) info[0] = 4;

			if ( nfev[0] >= maxfev ) info[0] = 3;

			if ( brackt[0] && stmax - stmin <= xtol * stmax ) info[0] = 2;

			if ( f <= ftest1 && Math.abs ( dg ) <= lbfgs_gtol * ( - dginit ) ) info[0] = 1;
			...
			if ( stage1 && f <= ftest1 && dg >= Math.min ( ftol , lbfgs_gtol ) * dginit ) stage1 = false;
